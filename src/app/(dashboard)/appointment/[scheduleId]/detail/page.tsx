...
export default function FixedScheduleDetailPage() {
    ...
    return <div className="flex flex-col flex-1 w-full bg-[#f9f9f9]">
        <Header
            leftChild={
        ...
        {isPending || !scheduleDetail ? (
            ...
            <div className="p-4">
                <ConfirmedTimeCard date={parseScheduleString(scheduleDetail?.scheduleId).date} time={parseScheduleString(scheduleDetail?.scheduleId).time} />
            </div>

        ) : (
            <div className="p-4">
                <ConfirmedPlaceCard placeName={scheduleDetail.placeName} placeAddress={scheduleDetail.placeName} />
            </div>
        )}
    </div>
}