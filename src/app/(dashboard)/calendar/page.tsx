...
export default function CalendarPage() {
    // (3) 현재 날짜(title)를 상태로 관리 (선택 사항이지만, 유연성을 위해 추천)
    ...
    // 3. datesSet 이벤트 핸들러: 달력 날짜가 변경될 때마다 실행됩니다.
    const handleDatesSet = (arg: { view: { title: React.SetStateAction<string>; }; }) => {
        ...
        <div className="w-full h-full bg-white custom-calendar-container">
            <div className="flex flex-col bg-white rounded-lg shadow-[0px_4px_10px_0px_rgba(0,0,0,0.04)] h-full">
                ...
                <div className="w-full flex items-center justify-between px-4 py-3 border-b border-gray-200">
                    {/* 이전 달 버튼 */}
                    <button onClick={handlePrevClick} className="p-1 text-gray-600 hover:text-gray-900 transition-colors">
                    ...
                    {/* 다음 달 버튼 */}
                    <button onClick={handleNextClick} className="p-1 text-gray-600 hover:text-gray-900 transition-colors">
